...
class BaseInvoiceDataset(Dataset):
    def __init__(self, ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings=False, transform=None, pre_transform=None):
        ...
        self.vit_model = ViTModel.from_pretrained('google/vit-base-patch16-224-in21k').to(device)
        self.error_logs = []
    ...
    def process_single_file(self, ocr_lines, ground_truth, file_name, regenerate_bert_embeddings):
        ...
        image_embeddings = self.get_image_embeddings(node_features, file_name, regenerate_bert_embeddings)
        node_features = [np.concatenate([bbox, emb]) for bbox, emb in zip(node_features, image_embeddings)]
    ...
    def get_image_embeddings(self, bboxes, file_name, regenerate_bert_embeddings):
        emb_file = os.path.join(self.bert_emb_dir, file_name.replace('.txt', '_image_embeddings.npy'))
        if not regenerate_bert_embeddings and os.path.exists(emb_file):
            embeddings = np.load(emb_file)
        else:
            images = [self.extract_image_from_bbox(bbox) for bbox in bboxes]
    ...
    def extract_image_from_bbox(self, bbox):
        # Dummy implementation - replace with actual image extraction
        # Assuming bbox = [x1, y1, x2, y2, x3, y3, x4, y4] and images are loaded elsewhere
        width = max(bbox[0], bbox[2], bbox[4], bbox[6]) - min(bbox[0], bbox[2], bbox[4], bbox[6])
...
def main():
    # Configuration
    num_node_features = 8 + 768  # Example: bbox coordinates + ViT embeddings
    ...
    bert_emb_dir = 'data3/vit_embeddings'
    ...
    if enable_training:
        ...
        for epoch in range(1, epochs + 1):
            ...
            remaining_time = (epochs - epoch) * avg_epoch_time / 3600  # Convert seconds to hours
            ...
            if log_training_reports and (epoch % 20 == 0 or epoch == epochs):
