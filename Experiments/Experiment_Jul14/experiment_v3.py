...
class ConfigurableGNN(torch.nn.Module):
    def __init__(self, num_node_features, num_classes, hidden_channels=64, num_gcn_layers=2, num_gat_layers=2, use_spatial_attention=False, num_attention_heads=1):
        ...
        if use_spatial_attention:
            self.spatial_attention = SpatialAttentionLayer(hidden_channels, hidden_channels, num_heads=num_attention_heads)
...
def main():
    ...
    model = ConfigurableGNN(num_node_features, num_classes, hidden_channels, num_gcn_layers, num_gat_layers, use_spatial_attention, num_attention_heads).to(device)
