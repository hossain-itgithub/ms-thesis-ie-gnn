...
class BaseInvoiceDataset(Dataset):
    def __init__(self, ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings=False, embedding_type='bert', transform=None, pre_transform=None):
        ...
        # Select tokenizer and model based on the embedding_type flag
        if self.embedding_type == 'labse':
            self.tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/LaBSE')
            self.embedding_model = AutoModel.from_pretrained('sentence-transformers/LaBSE').to(device)
        elif self.embedding_type == 'xmlroberta':
...
class InvoiceDataset(BaseInvoiceDataset):
    def __init__(self, ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings=False, embedding_type='bert', transform=None, pre_transform=None):
        super(InvoiceDataset, self).__init__(ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings, embedding_type, transform, pre_transform)


class TestInvoiceDataset(BaseInvoiceDataset):
    def __init__(self, ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings=False, embedding_type='bert', transform=None, pre_transform=None):
        super(TestInvoiceDataset, self).__init__(ocr_dir, gt_dir, error_dir, bert_emb_dir, regenerate_bert_embeddings, embedding_type, transform, pre_transform)
...
def main(resume_training=False, resume_config_path=None, resume_model_path=None):
    if resume_training:
        ...
        model_config = {
            ...
            "hidden_channels": 6000,
            "num_gcn_layers": 3,
            "num_gat_layers": 3,
            "use_spatial_attention": False,
            "num_gat_heads": 2,  # Example number of GAT heads
            "num_spatial_attention_heads": 1,  # Example number of spatial attention heads
            "learning_rate": 0.00008,
            "epochs": 400,
            "batch_size": 4,
            "regenerate_bert_embeddings": True,
            "log_training_reports": True,
            "enable_training": True,
            "enable_evaluation": True,
            "embedding_type": "labse"  # Set this to 'bert', 'xmlroberta', or 'labse'
        }
    ...
    dataset = InvoiceDataset(ocr_dir, gt_dir, error_dir, bert_emb_dir, model_config["regenerate_bert_embeddings"], model_config["embedding_type"])
    loader = DataLoader(dataset, batch_size=model_config["batch_size"], shuffle=True)

    # Create test dataset and dataloader
    test_dataset = TestInvoiceDataset(test_ocr_dir, test_gt_dir, error_dir, bert_emb_dir, model_config["regenerate_bert_embeddings"], model_config["embedding_type"])
...
if __name__ == "__main__":
    ...
    main(resume_training=resume_training, resume_config_path=resume_config_path, resume_model_path=resume_model_path)
