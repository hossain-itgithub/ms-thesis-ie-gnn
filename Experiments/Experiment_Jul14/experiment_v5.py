...
def load_checkpoint(model, optimizer, path):
    ...
    print(f"Checkpoint keys: {checkpoint.keys()}")  # Debugging line to print keys
...
class BaseInvoiceDataset(Dataset):
    ...
    def process_single_file(self, ocr_lines, ground_truth, file_name, regenerate_bert_embeddings):
        ...
        x = torch.tensor(np.array(node_features), dtype=torch.float)  # Convert to numpy array before tensor
...
def main():
    # Configuration
    resume_training = False  # Flag for resuming training
    config_path = 'logs/20240802_123929/model_config.json'  # Path to the configuration file
    checkpoint_path = 'logs/20240802_123929/invoice_gnn.pth'  # Path to the checkpoint file
    regenerate_bert_embeddings = False
    log_training_reports = True
    enable_training = True
    enable_evaluation = True

    if resume_training and os.path.exists(config_path):
        with open(config_path, 'r') as f:
            model_config = json.load(f)["Configuration"]
    else:
        ...
        model_config = {
            ...
            "num_gat_heads": 8,  # Example number of GAT heads
            "num_spatial_attention_heads": 2,  # Example number of spatial attention heads
    ...
    # Save model configuration if not resuming
    ...
    model = ConfigurableGNN(num_node_features, num_classes, hidden_channels, num_gcn_layers, num_gat_layers, use_spatial_attention, num_gat_heads, num_spatial_attention_heads).to(device)
