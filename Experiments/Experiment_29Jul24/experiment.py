...
def main():
    # Load configuration
    config_path = 'best_checkpoint/model_config.json'
    ...
    regenerate_bert_embeddings = config["regenerate_bert_embeddings"]
    log_training_reports = config["log_training_reports"]
    enable_training = config["enable_training"]
    enable_evaluation = config["enable_evaluation"]

    # File paths
    ocr_dir = 'ArabTrainDataset/box'
    gt_dir = 'ArabTrainDataset/entities'
    error_dir = 'ArabTrainDataset/error_files'
    bert_emb_dir = 'ArabTrainDataset/bert_embeddings'
    test_ocr_dir = 'ArabTestDataset/box'
    test_gt_dir = 'ArabTestDataset/entities'
    model_path = 'best_checkpoint/invoice_gnn.pth'  # Specify the path to your saved model

    # Flag to control training mode
    ...
    if resume_training and os.path.exists(model_path):
        ...
        with open(log_file, 'a') as f:
            f.write(f'Loaded model from {model_path}\n')
    ...
    if enable_training:
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % 20 == 0 or epoch == epochs:
                ...
                with open(log_file, 'a') as f:
                    f.write(f'Epoch: {epoch}, Loss: {train_loss}, Accuracy: {train_accuracy}\n')
                if log_training_reports:
                    ...
                    with open(log_file, 'a') as f:
                        f.write(f'Evaluation Loss: {eval_loss}, Accuracy: {accuracy}\n')
