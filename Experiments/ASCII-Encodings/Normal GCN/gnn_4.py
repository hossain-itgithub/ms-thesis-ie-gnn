...
# graph_data, tokens, labels = train_dataset[0]  # Replace 0 with any index to visualize different graphs
# if tokens is not None and labels is not None:
#     tokens = list(tokens)
#     labels = list(labels)
#     image_path = os.path.join(img_folder, train_dataset.image_files[0])
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.002, weight_decay=5e-8)
...
for epoch in range(100):  # Train for 100 epochs
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), f'gcn_epoch_{epoch + 1}.pth')

print("Training complete")
# Save the final model
torch.save(model.state_dict(), 'gcn_final.pth')
