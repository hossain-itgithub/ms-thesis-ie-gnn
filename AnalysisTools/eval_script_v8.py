...
from my_model7 import ConfigurableGNN, InvoiceDataset, device  # Adjust imports based on your file structure
...
def draw_bounding_boxes(image, boxes, labels, colors, class_names):
    ...
    exif = image.getexif()  # Correct method call
    orientation = exif.get(274) if exif else 1
    boxes = adjust_bounding_boxes_for_orientation(boxes, orientation, image.size)

    adjusted_boxes = []
    for box in boxes:
        # Debug: Print original box coordinates
        ...
        # Debug: Print adjusted coordinates before adding to list
        print(f"Adjusted coordinates: x_min={x_min}, y_min={y_min}, x_max={x_max}, y_max={y_max}")
        
        # Calculate height and check for issues
        height = y_max - y_min
        if height <= 0:
            print(f"Warning: Box height is {height}, skipping box. Coordinates: x_min={x_min}, y_min={y_min}, x_max={x_max}, y_max={y_max}")
            continue
    ...
    for box, label in zip(adjusted_boxes, labels):
        color = colors[label]
        x0, y0, x1, y1 = box
        
        print(f"Drawing box: x0={x0}, y0={y0}, x1={x1}, y1={y1} with color: {color}")
...
def main():
    log_dir = 'Analysis/CoRU_Results/Bert/WithoutSA'  # Replace with the actual directory containing log files
    ...
    ocr_dir = 'Datasets/CoRU/test/box'  # Replace with the actual OCR directory
    gt_dir = 'Datasets/CoRU/test/entities'  # Replace with the actual ground truth directory
    bert_emb_dir = 'Datasets/CoRU/test_embeddings'  # Replace with the actual BERT embeddings directory
    img_dir = 'Datasets/CoRU/test/img'  # Replace with the actual image directory
    ...
    for log_file in log_files:
        ...
        label = f'GCN: {config["num_gcn_layers"]}, GAT: {config["num_gat_layers"]}, SA: {config["use_spatial_attention"]}, HC: {config["hidden_channels"]}, LR: {learning_rate}, Embedding: {config.get("embedding_type", "bert")}'
        ...
        dataset = InvoiceDataset(
            ...
            gt_dir=gt_dir,
            error_dir=error_dir,
            bert_emb_dir=bert_emb_dir,
            regenerate_bert_embeddings=False,
            embedding_type=config.get("embedding_type", "bert")  # Pass the embedding_type to the dataset
        )
        ...
        model = ConfigurableGNN(
            ...
            num_gat_heads=config.get("num_gat_heads", 4),  # Ensure num_gat_heads is read from config
            num_spatial_attention_heads=config.get("num_spatial_attention_heads", 1)  # Ensure num_spatial_attention_heads is read from config
        ).to(device)
        ...
        for sample_data, _ in sample_data_for_plot:
            ...
            img_path = os.path.join(img_dir, sample_data.file_path[0].replace('.txt', '.jpg'))  # Ensure this gets the first element if it's a list and replace .txt with .jpg
            ...
            plt.savefig(os.path.join(predictions_dir, f'{sample_data.file_path[0].replace(".txt", ".pdf")}'), format='pdf')
            plt.close('all')
