...
def parse_log_file(log_file):
    ...
    for line in lines:
        if re.match(r'Epoch: \d+, Loss:', line):
            ...
            lr = float(parts[3].split(':')[1].strip()) if len(parts) > 3 else None
            epochs.append(epoch)
    ...
    return epochs, train_losses, train_accuracies, eval_losses, eval_accuracies, learning_rates
...
def main():
    ...
    # ocr_dir = 'ArabTestDataset/box'  # Replace with the actual OCR directory
    # gt_dir = 'ArabTestDataset/entities'  # Replace with the actual ground truth directory
    # bert_emb_dir = 'ArabTestDataset/bert_embeddings'  # Replace with the actual BERT embeddings directory
    # img_dir = 'ArabTestDataset/images'  # Replace with the actual image directory
    ...
    predictions_dir = os.path.join(os.path.dirname(log_file), 'predictions')
    ...
    for label, results in results_per_model.items():
        ...
        plot_confusion_matrix(y_true, y_pred, os.path.dirname(log_file), label)
        ...
        for sample_data, log_file in results['sample_data_for_plot']:
            ...
            plt.savefig(os.path.join(predictions_dir, f'{sample_data.file_path[0]}.png'))
            plt.close('all')
            #plt.show()
