...
from my_model6 import ConfigurableGNN, InvoiceDataset, device  # Adjust imports based on your file structure
...
def main():
    ...
    bert_emb_dir = 'Datasets/CoRU/embeddings'  # Replace with the actual BERT embeddings directory
    ...
    for log_file in log_files:
        ...
        label = f'GCN: {config["num_gcn_layers"]}, GAT: {config["num_gat_layers"]}, SA: {config["use_spatial_attention"]}, HC: {config["hidden_channels"]}, LR: {learning_rate}, XMLRoBERTa: {config.get("use_xmlroberta", False)}'
        ...
        dataset = InvoiceDataset(
            ...
            use_xmlroberta=config.get("use_xmlroberta", False)  # Pass the flag to the dataset
        )
